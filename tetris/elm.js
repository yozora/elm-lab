...
Elm.Data.make = function (_elm) {
   ...
   var TPiece = {ctor: "TPiece"};
   var LPiece = {ctor: "LPiece"};
   var JPiece = {ctor: "JPiece"};
   var SPiece = {ctor: "SPiece"};
   var ZPiece = {ctor: "ZPiece"};
   var Square = {ctor: "Square"};
   var Line = {ctor: "Line"};
   var Shadow = {ctor: "Shadow"};
...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Render2d = Elm.Render2d.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var defaultState = {_: {}
                      ,board: $Dict.fromList(_L.fromArray([{ctor: "_Tuple2"
                                                           ,_0: {ctor: "_Tuple2"
                                                                ,_0: 1
                                                                ,_1: 1}
                                                           ,_1: $Data.Red}
                                                          ,{ctor: "_Tuple2"
                                                           ,_0: {ctor: "_Tuple2"
                                                                ...
                                                                ,_1: 5}
                                                           ,_1: $Data.Blue}]))
                      ,cursor: $Maybe.Nothing
                      ,next: {ctor: "_Tuple2"
                             ,_0: $Data.Line
                             ,_1: $Data.Red}
                      ,paused: false};
   var main = $Render2d.render(defaultState.board);
   var emptyBoard = _L.fromArray([]);
...
Elm.Render2d = Elm.Render2d || {};
Elm.Render2d.make = function (_elm) {
   "use strict";
   _elm.Render2d = _elm.Render2d || {};
   if (_elm.Render2d.values)
   return _elm.Render2d.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var toColor = function (tcolor) {
      return function () {
         switch (tcolor.ctor)
         {case "Blue":
            return $Color.darkBlue;
            case "Cyan":
            return A3($Color.rgb,0,255,255);
            case "Green":
   ...
   var rows = 22;
   var blockDim = 40;
   var block = F2(function (colour,
   _v1) {
      return function () {
         switch (_v1.ctor)
         {case "_Tuple2":
            return function () {
                 var xx = (-1 * (cols / 2) + $Basics.toFloat(_v1._0)) * blockDim;
                 var yy = (-1 * (rows / 2) + $Basics.toFloat(_v1._1)) * blockDim;
                 return $Graphics$Collage.move({ctor: "_Tuple2"
                                               ,_0: xx
                                               ,_1: yy});
              }()($Graphics$Collage.move({ctor: "_Tuple2"
                                         ,_0: blockDim / 2
                                         ,_1: blockDim / 2})(A2($Graphics$Collage.filled,
              colour,
              $Graphics$Collage.square(blockDim))));}
         _U.badCase($moduleName,
         "between lines 30 and 34");
      }();
   });
   var buildScene = function (board) {
         ...
         return function () {
            ...
            {case "_Tuple2":
               return A2(block,
                 ...
                 _v5._0);}
            _U.badCase($moduleName,
            "on line 39, column 34 to 60");
         }();
      })($Dict.toList(board));
   };
   var backdrop = A2($Graphics$Collage.filled,
   $Color.black,
   ...
   var render = function (board) {
      return function () {
         var scene = buildScene(board);
         var trough = function () {
            var ys = $Array.toList(A2($Array.initialize,
            rows - 1,
            F2(function (x,y) {
               return x + y;
            })(1)));
            var xs = $Array.toList(A2($Array.initialize,
            cols,
            $Basics.identity));
            var ps = A2($Basics._op["++"],
            A2($List.map,
            F2(function (v0,v1) {
               return {ctor: "_Tuple2"
                      ,_0: v0
            ...
            xs)));
            return $List.map(block($Color.darkCharcoal))(ps);
         }();
         var forms = A2($Basics._op["++"],
         _L.fromArray([backdrop]),
         A2($Basics._op["++"],
         trough,
         scene));
